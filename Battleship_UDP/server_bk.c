...
int main() {
    ...
    while (TRUE) {
        ...
        ssize_t recvfrom_client;
        //Receive from client 1
        check_transfer(recvfrom_client = recvfrom(server_fd, buffer_c1, BUFFER_SIZE, 0, (struct sockaddr *) &client_1.client_addr, &client_1.addr_len),
                       "Receive From Error");
        buffer_c1[recvfrom_client] = '\0';
        printf("Client %s:%d: %s, %zd\n", inet_ntoa(client_1.client_addr.sin_addr), ntohs(client_1.client_addr.sin_port), buffer_c1, recvfrom_client);

        // Receive message from client 2
        check_transfer(recvfrom_client = recvfrom(server_fd, buffer_c2, BUFFER_SIZE, 0, (struct sockaddr *) &client_2.client_addr, &client_2.addr_len),
                       "Receive From Error");
        buffer_c2[recvfrom_client] = '\0';
        printf("Client %s:%d: %s, %zd\n", inet_ntoa(client_2.client_addr.sin_addr), ntohs(client_2.client_addr.sin_port), buffer_c2, recvfrom_client);
        printf("%zd\n", recvfrom_client);
